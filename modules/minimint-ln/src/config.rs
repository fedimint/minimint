...
impl GenerateConfig for LightningModuleConfig {
    ...
    async fn distributed_gen(
        ...
        mut rng: impl RngCore06 + CryptoRng,
...
/// FIXME copied from minimint/config
...
impl LightningModuleConfig {
    pub async fn distributed_threshold_gen<'a>(
        ...
        peers: &'a [PeerId],
        threshold: usize,
        rng: &'a mut (impl RngCore06 + CryptoRng),
    ) -> (Fr, PublicKeySet) {
...
fn test_g1_serde() {
    let commit =
        BivarPoly::random(1, &mut Rand07Compat(secp256k1::rand::thread_rng())).commitment();
    let g1 = ThresholdKeyGen::Verify(
        commit,
        G1::random(&mut Rand07Compat(secp256k1::rand::thread_rng())),
    );
...
fn test_threshold_sigs() {
    ...
    assert_eq!(
        threshold_pub_key.public_key_share(0),
